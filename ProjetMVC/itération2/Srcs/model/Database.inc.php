...
class Database {
	...
	private function checkNicknameAvailability($nickname) {
        ...
        $nick = $this->connection->query("SELECT nickname FROM users WHERE nickanme='$nickname'");
	...
	public function checkPassword($nickname, $password) {
		...
		if (checkNicknameValidity($nickname) === True && checkPasswordValidity($password) === True
	...
	public function addUser($nickname, $password) {
	  ...
	  if (checkPassword($nickname, $password) === True)
	  {
          $this->connection->exec();

          echo "Compte de $nickname ajouté avec succès";
          return True;
	  }
		  ...
		  {
		  	echo "Le pseudo doit contenir entre 3 et 10 lettres.";
		  }
		  ...
		  if (checkNicknameAvailability($nickname) ===  False)
		  {
